...
pub const ROARING_VERSION: &[u8; 6] = b"5.2.2\0";
pub const ROARING_VERSION_MAJOR: _bindgen_ty_1 = 5;
...
pub const BITSET_CONTAINER_SIZE_IN_WORDS: _bindgen_ty_7 = 1024;
pub const BITSET_UNKNOWN_CARDINALITY: _bindgen_ty_7 = -1;
...
#[doc = " An opaque 64-bit Roaring bitmap. Create one with `roaring64_bitmap_create()`\n and release it with `roaring64_bitmap_free()`."]
pub type roaring64_bitmap_t = roaring64_bitmap_s;
#[doc = " Internal leaf type, exposed only for use inside `roaring64_bulk_context_t`.\n Callers should treat it as opaque."]
...
extern "C" {
    #[doc = " Returns true if all values in the range [min, max] are present."]
    pub fn roaring64_bitmap_contains_range_closed(
        r: *const roaring64_bitmap_t,
        min: u64,
        max: u64,
    ) -> bool;
...
extern "C" {
    ...
    pub fn roaring64_iterator_read_backward(
        it: *mut roaring64_iterator_t,
        buf: *mut u64,
        count: u64,
    ) -> u64;
...
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct roaring64_range_closed_s {
    pub min: u64,
    pub max: u64,
}
pub type roaring64_range_closed_t = roaring64_range_closed_s;
extern "C" {
    #[doc = " Reads next ${count} ranges from iterator into user-supplied ${buf}.\n A range is defined as a maximal interval of consecutive values.\n For example, the set {1,2,3,5,6} contains two ranges: [1..3] and [5..6].\n Each range is represented as a struct {min,max}, both endpoints included.\n Consecutive values that span internal container boundaries are merged into\n a single range.\n\n Returns the number of read ranges.\n This number can be smaller than ${count}, which means that the iterator is\n drained.\n\n This function can be used together with other iterator functions.\n  - first range will start with the current iterator value\n  - after the function returns, the iterator is positioned at the next element\n    after the end of the last returned range, or has_value is false if\n    the bitmap is exhausted."]
    pub fn roaring64_iterator_read_ranges(
        it: *mut roaring64_iterator_t,
        buf: *mut roaring64_range_closed_t,
        count: usize,
    ) -> usize;
}
extern "C" {
    ...
    pub fn roaring64_iterator_read_prev_ranges(
        it: *mut roaring64_iterator_t,
        buf: *mut roaring64_range_closed_t,
        count: usize,
    ) -> usize;
