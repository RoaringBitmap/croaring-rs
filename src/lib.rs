...
impl Bitmap {
    ...
    pub fn serialize(&self) -> &[u8] {
        let mut buffer: Vec<i8> = Vec::with_capacity(self.get_serialized_size_in_bytes());

        unsafe {
            ffi::roaring_bitmap_portable_serialize(self.bitmap, buffer.as_mut_ptr());

            std::slice::from_raw_parts(buffer.as_ptr() as *const u8,
                                       buffer.capacity() * std::mem::size_of::<i8>())
        }
    }
    ...
    pub fn deserialize(buffer: &[u8]) -> Self {
        unsafe {
            let bytebuf: &[i8] = std::mem::transmute(buffer);

            Bitmap  { bitmap: ffi::roaring_bitmap_portable_deserialize(bytebuf.as_ptr()) }
        }
    }
